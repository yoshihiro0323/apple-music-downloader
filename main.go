...
func writeCover(sanAlbumFolder, name string, url string) (string, error) {
	...
	// ユーザーが指定したファイル名がある場合はそれを使用
	if *output_filename != "" {
		coverName = forbiddenNames.ReplaceAllString(*output_filename, "_")
		fmt.Println("Custom filename applied to cover:", *output_filename)
	} else {
		coverName = name
	}

	covPath := filepath.Join(sanAlbumFolder, coverName+"."+Config.CoverFormat)
	if Config.CoverFormat == "original" {
		ext := strings.Split(url, "/")[len(strings.Split(url, "/"))-2]
		ext = ext[strings.LastIndex(ext, ".")+1:]
		covPath = filepath.Join(sanAlbumFolder, coverName+"."+ext)
	}
...
func downloadTrack(trackNum int, trackTotal int, meta *structs.AutoGenerated, track structs.TrackData, albumId, token, storefront, mediaUserToken, sanAlbumFolder, Codec string, covPath string) {
	...
	if track.Type == "music-videos" {
		fmt.Print("MV Download is not available")
		counter.Success++
		return
	}
	...
	if *output_filename != "" {
		// ユーザーが指定したファイル名を使用
		filename = fmt.Sprintf("%s.m4a", forbiddenNames.ReplaceAllString(*output_filename, "_"))
		fmt.Println("Custom filename applied to track:", *output_filename)
	} else {
	...
	if len(mediaUserToken) > 50 {
		fmt.Println("⚠️ 歌詞機能は無効化されています。歌詞のダウンロードはスキップします。")
	}
...
func rip(albumId string, token string, storefront string, mediaUserToken string, urlArg_i string) error {
	...
	if Config.SaveArtistCover && !(strings.Contains(albumId, "pl.")) {
		if len(meta.Data[0].Relationships.Artists.Data) > 0 {
			var artistCoverName string
			if *output_filename != "" {
				artistCoverName = forbiddenNames.ReplaceAllString(*output_filename, "_") + "_artist"
			} else {
				artistCoverName = "folder"
			}
			_, err = writeCover(saveFolder, artistCoverName, meta.Data[0].Relationships.Artists.Data[0].Attributes.Artwork.Url)
			if err != nil {
	...
	//get album cover
	covPath, err := writeCover(sanAlbumFolder, albumId, meta.Data[0].Attributes.Artwork.URL)
	if err != nil {
		fmt.Println("Failed to write cover.")
	}
	...
	if Config.SaveAnimatedArtwork && meta.Data[0].Attributes.EditorialVideo.MotionDetailSquare.Video != "" {
		...
		// アニメーションアートワークのファイル名
		var animatedArtworkName string
		if *output_filename != "" {
			animatedArtworkName = forbiddenNames.ReplaceAllString(*output_filename, "_")
		} else {
			animatedArtworkName = "square_animated_artwork"
		}
		...
		if err != nil {
			...
			squareArtworkPath := filepath.Join(sanAlbumFolder, animatedArtworkName+"_square.mp4")
			exists, err := fileExists(squareArtworkPath)
			if err != nil {
			...
			if exists {
				...
				cmd := exec.Command("ffmpeg", "-loglevel", "quiet", "-y", "-i", motionvideoUrlSquare, "-c", "copy", squareArtworkPath)
				if err := cmd.Run(); err != nil {
		...
		if Config.EmbyAnimatedArtwork {
			...
			if *output_filename != "" {
				folderName = forbiddenNames.ReplaceAllString(*output_filename, "_")
			} else {
				folderName = "folder"
			}

			cmd3 := exec.Command("ffmpeg", "-i", filepath.Join(sanAlbumFolder, animatedArtworkName+"_square.mp4"), "-vf", "scale=440:-1", "-r", "24", "-f", "gif", filepath.Join(sanAlbumFolder, folderName+".jpg"))
			if err := cmd3.Run(); err != nil {
				fmt.Printf("animated artwork square to gif err: %v\n", err)
			}
		}

		// アニメーションアートワーク（縦長版）のファイル名
		var tallAnimatedArtworkName string
		if *output_filename != "" {
			tallAnimatedArtworkName = forbiddenNames.ReplaceAllString(*output_filename, "_")
		} else {
			tallAnimatedArtworkName = "tall_animated_artwork"
		}
		...
		if err != nil {
			...
			tallArtworkPath := filepath.Join(sanAlbumFolder, tallAnimatedArtworkName+"_tall.mp4")
			exists, err := fileExists(tallArtworkPath)
			if err != nil {
			...
			if exists {
				...
				cmd := exec.Command("ffmpeg", "-loglevel", "quiet", "-y", "-i", motionvideoUrlTall, "-c", "copy", tallArtworkPath)
				if err := cmd.Run(); err != nil {
...
func main() {
	...
	for {
		...
		// エラーが発生した場合は終了
		fmt.Println("エラーが発生したため、処理を終了します。")
		break
	}
}
